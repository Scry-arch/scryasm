...
/// Executes the program with the given input on stdin, returning a testable assert.
...
fn to_encoded(instructions: &Vec<Instruction>) -> Vec<u8> {
	instructions.iter().fold(Vec::new(), |mut v, b| {v.extend_from_slice(&b.encode().to_le_bytes()); v})
}
...
#[quickcheck]
fn text_instruction_to_text_hex(first: Instruction, mut instructions: Vec<Instruction>) -> Result<(), AssertError> {
...
/// and output it raw
#[quickcheck]
fn text_instruction_to_raw(first: Instruction, mut instructions: Vec<Instruction>) -> Result<(), AssertError> {
	// Ensure there is at least 1 instruction
	instructions.push(first);
